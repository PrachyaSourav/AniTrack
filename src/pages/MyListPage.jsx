...
export default function MyListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    items.sort((a, b) => {
      if (sortBy === "addedAt") return (b.addedAt || 0) - (a.addedAt || 0);
      if (sortBy === "rating") return (b.rating || 0) - (a.rating || 0);
      if (sortBy === "title") return a.title.localeCompare(b.title);
      if (sortBy === "progress") { const ap = a.total > 0 ? a.progress/a.total : 0; const bp = b.total > 0 ? b.progress/b.total : 0; return bp - ap; }
      return 0;
    });
    return items;
  ...
  const counts = useMemo(() => {
    const c = {}; STATUS_FILTERS.forEach((s) => { c[s] = s === "All" ? list.length : list.filter((x) => x.status === s).length; }); return c;
  }, [list]);

  return (
    <div className="max-w-5xl mx-auto px-4 py-8 fade-up">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-1" style={{ color: "var(--text)", fontFamily: "'Syne', sans-serif" }}>My List</h1>
        <p className="text-sm" style={{ color: "var(--text3)" }}>{list.length} item{list.length !== 1 ? "s" : ""} tracked</p>
      </div>
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {STATUS_FILTERS.map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)} className={`chip flex items-center gap-1.5 ${statusFilter === s ? "active" : ""}`}>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20" style={{ color: "var(--text3)" }}>
          <p className="text-4xl mb-3">{list.length === 0 ? "📋" : "🔍"}</p>
          <p className="text-sm">{list.length === 0 ? "Your list is empty. Head to Discover to add something!" : "No items match your filters."}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3 stagger">
          {filtered.map((item) => <ListItem key={item.id} item={item} onEdit={() => setModalItem(item)} />)}
        </div>
      )}
