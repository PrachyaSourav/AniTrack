...
export default function MyListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    items.sort((a, b) => {
      ...
      if (sortBy === "progress") {
        const aPct = a.total > 0 ? a.progress / a.total : 0;
        const bPct = b.total > 0 ? b.progress / b.total : 0;
        return bPct - aPct;
      }
      return 0;
    });
  ...
  const counts = useMemo(() => {
    ...
    STATUS_FILTERS.forEach((s) => {
      c[s] = s === "All" ? list.length : list.filter((x) => x.status === s).length;
    });
    return c;
  }, [list]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 fade-up">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm">{list.length} item{list.length !== 1 ? "s" : ""} tracked</p>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {STATUS_FILTERS.map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`chip flex items-center gap-1.5 ${statusFilter === s ? "active" : ""}`}>
              {s}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-white/30">
          ...
          <p className="text-sm">
            {list.length === 0 ? "Your list is empty. Head to Discover to add something!" : "No items match your filters."}
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-3 stagger">
          {filtered.map((item) => (
            <ListItem key={item.id} item={item} onEdit={() => setModalItem(item)} />
          ))}
        </div>
      )}
