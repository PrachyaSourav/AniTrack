...
const API_SOURCE = {
  ...
  manhwa: "Jikan (MyAnimeList)",
  manhua: "Jikan (MyAnimeList)",
  donghua: "Jikan (MyAnimeList)",
  lightnovel: "Jikan (MyAnimeList)",
  webnovel: "Jikan (MyAnimeList)",
  kdrama: "OMDB (IMDb)",
  movie: "OMDB (IMDb)",
  show: "OMDB (IMDb)",
};
...
export default function DiscoverPage() {
  ...
  async function handleSearch(q = query, t = type) {
    ...
    try {
      ...
      setResults(res);
      if (res.length === 0) setError("No results found. Try a specific title like Squid Game or Goblin.");
    } catch {
  ...
  function handleQueryChange(e) {
    ...
    debounceRef.current = setTimeout(() => handleSearch(val, type), 600);
  }
