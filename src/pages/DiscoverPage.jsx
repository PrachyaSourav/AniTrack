...
export default function DiscoverPage() {
  ...
  useEffect(() => { handleSearch("", "anime", 1); }, []);
  ...
  async function handleSearch(q = query, t = type, p = 1) {
    ...
    try {
      ...
      if (p === 1) setResults(res); else setResults((prev) => [...prev, ...res]);
      setPage(p);
  ...
  function handleTypeChange(t) {
    setType(t); setQuery(""); setResults([]); setPage(1); setHasMore(false);
    handleSearch("", t, 1);
  }
  ...
  const handleAIRecommend = async () => {
    ...
    try {
      ...
      if (!response.ok || data.error) throw new Error(data.debug || data.detail || data.error || "API error");
      ...
      const withImages = await Promise.allSettled(
        suggestions.map(async (s) => {
          const results = await searchMedia(s.title, TYPE_MAP[s.type] || "anime", 1);
          ...
          return match ? { ...match, aiReason: s.reason, type: s.type } :
            { id: Math.random(), title: s.title, type: s.type, img: "", score: 0, aiReason: s.reason };
        })
      );
      const filtered = withImages.filter((r) => r.status === "fulfilled" && r.value).map((r) => r.value);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 fade-up">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-1" style={{ color: "var(--text)", fontFamily: "'Syne', sans-serif" }}>Discover</h1>
        <p className="text-sm" style={{ color: "var(--text3)" }}>Search anime, manga, novels, K-dramas, movies, and more.</p>
      </div>

      <div className="flex items-center gap-3 mb-5">
        <button onClick={() => setShowAI((v) => !v)}
          className="flex items-center gap-2 text-sm px-4 py-2 rounded-full border transition-all duration-150"
          style={{ borderColor: showAI ? "var(--accent)" : "var(--border2)", color: showAI ? "var(--accent)" : "var(--text3)", background: showAI ? "color-mix(in srgb, var(--accent) 10%, transparent)" : "transparent" }}>
          ✨ AI Recommend
        </button>
        ...
        <div className="panel mb-6 fade-up" style={{ borderColor: "color-mix(in srgb, var(--accent) 30%, transparent)" }}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleAIRecommend} disabled={aiLoading || !aiQuery.trim()}
              ...
              {aiLoading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Thinking...</> : "Ask AI"}
            </button>
          </div>
          ...
          {aiResults.length > 0 && (
            <div className="mt-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 stagger">
                {aiResults.map((item) => (
                  <div key={`ai-${item.id}`} className="media-card group cursor-pointer flex flex-col" onClick={() => setModalItem(item)}>
                    <div className="relative aspect-[2/3] overflow-hidden" style={{ background: "var(--bg3)" }}>
                      {item.img ? <img src={item.img} alt={item.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" onError={(e) => { e.target.style.display = "none"; }} /> :
                        <div className="w-full h-full flex items-center justify-center text-3xl" style={{ color: "var(--text3)" }}>🎬</div>}
                      <div className="absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-md" style={{ background: "color-mix(in srgb, var(--accent) 80%, transparent)", color: "var(--bg)" }}>✨ AI</div>
                    </div>
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input type="text" value={query} onChange={handleQueryChange}
            ...
            placeholder={`Search ${currentType?.label || ""}...`} className="input pl-10" />
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {TYPES.map((t) => (
          <button key={t.value} onClick={() => handleTypeChange(t.value)} className={`chip flex items-center gap-1.5 ${type === t.value ? "active" : ""}`}>
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 stagger">
          {[...Array(10)].map((_, i) => <div key={i}><div className="skeleton aspect-[2/3] rounded-xl mb-2" /><div className="skeleton h-3 w-3/4 rounded mb-1" /><div className="skeleton h-3 w-1/2 rounded" /></div>)}
        </div>
      ) : error || results.length === 0 ? (
        <div className="text-center py-20" style={{ color: "var(--text3)" }}>
          <p className="text-4xl mb-3">🔎</p>
          <p className="text-sm">{error || "No results found. Try a specific title!"}</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 stagger">
            {results.map((item) => <MediaCard key={`${item.type}-${item.id}`} item={item} onAddClick={setModalItem} />)}
          </div>
          {hasMore && (
            <div className="flex justify-center mt-8">
              <button onClick={() => handleSearch(query, type, page + 1)} disabled={loadingMore} className="btn-ghost flex items-center gap-2 disabled:opacity-50">
                {loadingMore ? <><span className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />Loading more...</> : "Load more →"}
              </button>
            </div>
          )}
        </>
      )}
      {!loading && results.length > 0 && <p className="mt-6 text-center text-xs" style={{ color: "var(--text3)" }}>📡 Data from {API_SOURCE[type]}</p>}
