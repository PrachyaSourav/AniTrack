...
function TrendingWidget({ items, loading, title, emoji, linkTo }) {
  return (
    <div className="panel">
      <div className="flex items-center justify-between mb-4">
        <p className="section-label">{emoji} {title}</p>
        <Link to={linkTo} className="text-xs text-accent/70 hover:text-accent transition-colors">
          See all →
        </Link>
      </div>
      {loading ? (
        <div className="flex gap-3 overflow-hidden">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="flex-shrink-0 w-20">
              <div className="skeleton aspect-[2/3] rounded-lg mb-1.5" />
        ...
        <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-none">
          {items.slice(0, 8).map((item) => (
            <Link key={item.id} to="/trending"
              className="flex-shrink-0 w-20 group cursor-pointer">
              <div className="aspect-[2/3] rounded-lg overflow-hidden bg-surface-3 mb-1.5 relative">
                {item.img ? (
                ...
                {item.score > 0 && (
                  <div className="absolute bottom-1 left-1 bg-black/70 text-yellow-400 text-[9px] font-bold px-1 rounded">
                    ★{item.score}
                  </div>
                )}
              </div>
              <p className="text-[10px] text-white/60 leading-tight line-clamp-2 group-hover:text-white transition-colors">
                {item.title}
              </p>
            </Link>
          ))}
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 fade-up">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm">{season} {year} season is airing now</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 stagger">
        ...
        <StatCard label="Completed" value={stats.completed}
          sub={`${stats.total > 0 ? Math.round(stats.completed / stats.total * 100) : 0}% of list`} />
        <StatCard label="Watching now" value={stats.watching} />
        <StatCard label="Avg rating" value={stats.avgRating}
          sub={stats.avgRating !== "—" ? "out of 10" : "Rate some items!"} />
      </div>

      {/* Trending widgets */}
      <div className="flex flex-col gap-4 mb-8">
        <TrendingWidget items={trending} loading={trendingLoading}
          title="Trending Anime" emoji="🔥" linkTo="/trending" />
        <TrendingWidget items={airing} loading={airingLoading}
          title={`Airing This Season — ${season} ${year}`} emoji="📡" linkTo="/trending" />
      </div>
