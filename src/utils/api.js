...
async function searchKDrama(query) {
  ...
  try {
    ...
    const allResults=await Promise.allSettled(searches.map(q=>fetch(`${OMDB}/?apikey=${OMDB_KEY}&s=${encodeURIComponent(q)}&type=series`).then(r=>r.json())));
    const imdbIds=[];
    allResults.forEach(r=>{if(r.status==="fulfilled"&&r.value.Response!=="False")(r.value.Search||[]).slice(0,query.trim()?10:3).forEach(x=>{if(!imdbIds.includes(x.imdbID))imdbIds.push(x.imdbID);});});
    const details=await Promise.allSettled(imdbIds.slice(0,12).map(id=>fetch(`${OMDB}/?apikey=${OMDB_KEY}&i=${id}&plot=short`).then(r=>r.json())));
    return details.filter(r=>{if(r.status!=="fulfilled")return false;const d=r.value;if(d.Response==="False")return false;const hasPoster=d.Poster&&d.Poster!=="N/A"&&d.Poster.startsWith("http");if(!hasPoster)return false;if(query.trim())return true;return d.Country?.includes("South Korea")||d.Language?.includes("Korean");}).map(r=>fromOmdb(r.value,"K-Drama"));
  } catch(e){return[];}
}
